...
const updateCompleteProfile = async (req, res) => {
  try {
    ...
    if (Array.isArray(certifications)) {
      ...
      for (const cert of certifications) {
        if (cert.id && existingIds.includes(cert.id)) {
    ...
    if (Array.isArray(publications)) {
      ...
      const existingIds = existingPubs.map(p => p.id);
      ...
      for (const pub of publications) {
        if (pub.id && existingIds.includes(pub.id)) {
    ...
    if (Array.isArray(projects)) {
      ...
      for (const proj of projects) {
        if (proj.id && existingIds.includes(proj.id)) {
    ...
    if (Array.isArray(awards)) {
      ...
      const existingIds = existingAwards.map(a => a.id);
      ...
      for (const award of awards) {
        if (award.id && existingIds.includes(award.id)) {
